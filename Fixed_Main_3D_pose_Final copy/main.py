...
def crop_torso_mesh(mesh, skeleton_points):
    """
    메시를 토르소 형태로 자릅니다 (팔과 다리 일부 제거).
    - 다리: 무릎까지만 남김
    - 팔: 팔꿈치보다 약간 위쪽에서 자름
    ...
    Returns:
        o3d.geometry.TriangleMesh: 잘린 토르소 메시
    """
    if mesh is None or not skeleton_points:
        return mesh
    
    print("\n=== 토르소 메시 생성 ===")
    
    # 메시 복사
    torso_mesh = copy.deepcopy(mesh)
    vertices = np.asarray(torso_mesh.vertices)
    
    # 바운딩 박스 제거를 위한 이상치 제거
    # 신체 중심점 계산
    body_center = np.mean(vertices, axis=0)
    distances = np.linalg.norm(vertices - body_center, axis=1)
    median_distance = np.median(distances)
    std_distance = np.std(distances)
    
    # 중심에서 너무 먼 점들 제거 (정사각형 박스들)
    outlier_threshold = median_distance + 2.5 * std_distance
    valid_mask = distances < outlier_threshold
    
    print(f"  이상치 제거: {np.sum(~valid_mask)} 버텍스 제거됨")
    
    # 관절 위치 가져오기
    knee_left = skeleton_points.get("KNEE_LEFT")
    knee_right = skeleton_points.get("KNEE_RIGHT")
    elbow_left = skeleton_points.get("ELBOW_LEFT")
    elbow_right = skeleton_points.get("ELBOW_RIGHT")
    shoulder_left = skeleton_points.get("SHOULDER_LEFT")
    shoulder_right = skeleton_points.get("SHOULDER_RIGHT")
    
    # 다리 자르기 (무릎 위치 기준)
    if knee_left is not None and knee_right is not None:
        knee_y = (knee_left[1] + knee_right[1]) / 2
        # 무릎보다 약간 아래까지만 남김
        leg_cut_y = knee_y - 30  # 무릎 아래 30mm까지
        valid_mask &= vertices[:, 1] >= leg_cut_y
        print(f"  다리 자르기: Y <= {leg_cut_y:.1f} 제거 (무릎 기준)")
    
    # 팔 자르기 (팔꿈치 기준, 더 짧게)
    if elbow_left is not None and shoulder_left is not None:
        # 팔꿈치와 어깨 사이의 중간 지점에서 자름
        cut_left_y = (elbow_left[1] + shoulder_left[1]) / 2 + 30  # 어깨 쪽으로 더 가까이
        cut_left_x = elbow_left[0]
        # 왼쪽 팔 영역의 버텍스 제거
        left_arm_mask = (vertices[:, 0] < cut_left_x - 50) & (vertices[:, 1] < cut_left_y)
        valid_mask &= ~left_arm_mask
        print(f"  왼쪽 팔 자르기: X < {cut_left_x - 50:.1f}, Y < {cut_left_y:.1f} 제거")
    
    if elbow_right is not None and shoulder_right is not None:
        # 팔꿈치와 어깨 사이의 중간 지점에서 자름
        cut_right_y = (elbow_right[1] + shoulder_right[1]) / 2 + 30  # 어깨 쪽으로 더 가까이
        cut_right_x = elbow_right[0]
        # 오른쪽 팔 영역의 버텍스 제거
        right_arm_mask = (vertices[:, 0] > cut_right_x + 50) & (vertices[:, 1] < cut_right_y)
        valid_mask &= ~right_arm_mask
        print(f"  오른쪽 팔 자르기: X > {cut_right_x + 50:.1f}, Y < {cut_right_y:.1f} 제거")
    
    # 유효한 버텍스만 선택
    valid_vertices = vertices[valid_mask]
    vertex_map = np.cumsum(valid_mask) - 1  # 새로운 인덱스 매핑
    vertex_map[~valid_mask] = -1
    
    # 삼각형 필터링
    triangles = np.asarray(torso_mesh.triangles)
    valid_triangles = []
    for tri in triangles:
        if valid_mask[tri[0]] and valid_mask[tri[1]] and valid_mask[tri[2]]:
            new_tri = [vertex_map[tri[0]], vertex_map[tri[1]], vertex_map[tri[2]]]
            valid_triangles.append(new_tri)
    
    # 새 메시 생성
    torso_mesh.vertices = o3d.utility.Vector3dVector(valid_vertices)
    torso_mesh.triangles = o3d.utility.Vector3iVector(np.array(valid_triangles))
    
    # 법선 재계산
    torso_mesh.compute_vertex_normals()
    
    print(f"  토르소 메시 생성 완료:")
    print(f"    원본 버텍스: {len(vertices)} → 토르소 버텍스: {len(valid_vertices)}")
    print(f"    원본 삼각형: {len(triangles)} → 토르소 삼각형: {len(valid_triangles)}")
    
    return torso_mesh


def visualize_results(merged_cloud, mesh, skeleton_pcd, skeleton_cylinders, skeleton_points=None):
    """
    결과를 시각화합니다. (토르소 형태로 자른 메시 표시)
    ...
    Args:
        ...
        skeleton_pcd (o3d.geometry.PointCloud): 스켈레톤 포인트 클라우드
        skeleton_cylinders (list): 스켈레톤 연결선 실린더 리스트
        skeleton_points (dict): 스켈레톤 포인트 딕셔너리 (토르소 자르기용)
    """
    print("\n=== 3D 시각화 ===")
    
    # 메시를 토르소 형태로 자르기
    if mesh is not None and skeleton_points is not None:
        mesh = crop_torso_mesh(mesh, skeleton_points)
    
    # 시각화 창 생성
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name="3D Pose Analysis - Torso Model", width=1024, height=768)
    
    # 메시 추가 (토르소)
    if mesh is not None:
        mesh.paint_uniform_color([0.7, 0.75, 0.85])  # 밝은 회색-파랑 색상
    ...
    opt.mesh_show_wireframe = False  # 와이어프레임 비활성화
...
def main():
    ...
    try:
        ...
        # 6단계: 결과 시각화 (토르소 형태로)
        visualize_results(merged_cloud, mesh, skeleton_pcd, skeleton_cylinders, skeleton_points)
