...
def create_point_cloud_from_depth(depth_map, view):
    ...
    valid_mask = valid_depths > depth_threshold
    valid_points = points[valid_mask]
    
    print(f"{view} 뷰: 기본 임계값 적용 후 {len(valid_points)} 포인트")
...
def visualize_3d_pose():
    ...
    if "back" in point_clouds:
        ...
        if left_aligned is not None and len(left_aligned.points) > 0:
            side_targets.append(left_aligned)
            print(f"  좌측 정렬 결과: {len(left_aligned.points)} 포인트 사용")
        if right_aligned is not None and len(right_aligned.points) > 0:
            side_targets.append(right_aligned)
            print(f"  우측 정렬 결과: {len(right_aligned.points)} 포인트 사용")

        if len(side_targets) == 0:
            print("  경고: 좌/우 정렬 결과가 없어 후면 정렬을 건너뜁니다.")
            print("  후면을 정면에 직접 정렬합니다.")
            # 대안으로 정면에 정렬
            params_align = {
                'voxel_coarse': 5.0,
                'voxel_list': [20.0, 10.0, 5.0],
            ...
            back_aligned = align_point_clouds_v2(point_clouds["back"], front_target, params=params_align)
            aligned_clouds.append(back_aligned)
        else:
            # 좌/우 정렬 결과들을 하나의 타겟으로 병합
            side_target = o3d.geometry.PointCloud()
            st_points = []
            st_colors = []

            for side_pcd in side_targets:
                st_points.extend(np.asarray(side_pcd.points))
                if side_pcd.has_colors():
                    st_colors.extend(np.asarray(side_pcd.colors))
            ...
            print(f"  좌/우 병합 타겟 생성: {len(side_target.points)} 포인트")
            print(f"  후면 소스: {len(point_clouds['back'].points)} 포인트")

            # 후면을 좌/우 타겟에 정렬 (더 관대한 파라미터 사용)
            params_align = {
                'voxel_coarse': 8.0,  # 더 큰 초기 voxel 크기
                'voxel_list': [30.0, 15.0, 8.0],  # 더 큰 voxel 범위
                'ransac_iter': 40000,  # 더 많은 RANSAC 반복
                'use_cpd': False,
                'cpd_params': {'max_points':2000, 'cpd_beta':2.0, 'cpd_lambda':2.0, 'cpd_iter':50},
                'fitness_threshold_accept': 0.015,  # 더 관대한 임계값
                'force_cpd': False,
                'allow_rotation': True,  # 후면은 회전 허용
                'allow_small_rotation': False
            }
            
            print("  후면을 좌/우 병합 타겟에 정렬 중...")
            back_aligned = align_point_clouds_v2(point_clouds["back"], side_target, params=params_align)
            
            if back_aligned is not None:
                print(f"  후면 정렬 완료: {len(back_aligned.points)} 포인트")
                aligned_clouds.append(back_aligned)
            else:
                print("  후면 정렬 실패, 원본 후면 사용")
                aligned_clouds.append(point_clouds["back"])
