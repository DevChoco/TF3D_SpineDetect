...
def create_point_cloud_from_depth(depth_map, view):
    ...
    # 모든 포인트를 사용 (마스킹 제거)
    valid_points = points.reshape(-1, 3)
    
    # 기본 깊이 임계값 적용 (매우 낮은 값만 제외)
    depth_values = depth_map.flatten()
    depth_threshold = 0.01
    valid_mask = depth_values > depth_threshold
    valid_points = valid_points[valid_mask]
    
    print(f"{view} 뷰: 총 {len(points.reshape(-1, 3))} 포인트, 유효한 포인트 {len(valid_points)} 포인트")
