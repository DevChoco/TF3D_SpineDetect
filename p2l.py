...
def create_point_cloud_from_depth(depth_map, view):
    ...
    if view == "front":
        ...
        points = np.stack([depth_map * scale * 2.4, -y, -x], axis=-1)  # 우측 깊이 2배
    elif view == "left":
        points = np.stack([-depth_map * scale * 2.4, -y, x], axis=-1)  # 좌측 깊이 2배
    elif view == "back":
        points = np.stack([-x, -y, -depth_map * scale], axis=-1)
...
def align_point_clouds(source, target, threshold=10, use_preprocessing=True):
    ...
    print(f"  Point-to-Plane ICP 정렬 시작: source {len(source.points)}, target {len(target.points)} 포인트")
    ...
    if not source_processed.has_normals():
        ...
        source_processed.orient_normals_consistent_tangent_plane(k=15)
    ...
    if not target_processed.has_normals():
        ...
        target_processed.orient_normals_consistent_tangent_plane(k=15)
    
    # 초기 변환 행렬
    init_transformation = np.eye(4)
    
    # 1단계: 거친 정렬 (Point-to-Point ICP)
    print("  1단계: Point-to-Point ICP (초기 정렬)...")
    ...
    # 2단계: 정교한 정렬 (Point-to-Plane ICP - 주요 정렬 단계)
    print("  2단계: Point-to-Plane ICP (정밀 정렬)...")
...
def multi_scale_icp(source, target, voxel_list=[20.0, 10.0, 5.0], use_point_to_plane=True, max_iter_per_scale=[50,50,100]):
    """voxel-based multi-scale Point-to-Plane ICP: coarse -> fine
    ...
    print(f"  Multi-scale Point-to-Plane ICP 시작 (레벨: {len(voxel_list)})")
    ...
    for i, voxel in enumerate(voxel_list):
        ...
        # Point-to-Plane ICP를 위한 법선 벡터 계산 (항상 수행)
        print(f"    레벨 {i+1}/{len(voxel_list)} (voxel={voxel}): 법선 계산 중...")
        ...
        src_down.orient_normals_consistent_tangent_plane(k=15)
        tgt_down.orient_normals_consistent_tangent_plane(k=15)

        distance_threshold = voxel * 1.5
        
        # 항상 Point-to-Plane 사용 (더 정밀한 정렬)
        ...
        final_result = result
        print(f"    레벨 {i+1} 완료: fitness={result.fitness:.4f}, rmse={result.inlier_rmse:.4f}")

    if final_result is None:
        # 다운샘플 결과가 빈 경우 원본으로 1회 시도
        print("    다운샘플 실패, 원본으로 Point-to-Plane ICP 시도...")
        source.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=10, max_nn=30))
        target.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=10, max_nn=30))
...
def align_point_clouds_v2(source, target, params=None):
    """개선된 정렬: multi-scale Point-to-Plane ICP (FPFH 제거)
       params: dict으로 옵션 전달
    """
    ...
    if allow_rotation:
        print("  회전 허용 모드: Multi-scale Point-to-Plane ICP 수행")
        # Multi-scale ICP (회전 허용)
...
def align_point_clouds_all(point_clouds):
    """포인트 클라우드 정렬 (Point-to-Plane ICP, FPFH 제거)"""
    print(f"\n=== Point-to-Plane ICP 기반 포인트 클라우드 정렬 단계 ===")
...
def save_mesh_files(mesh, output_dir, base_name="body_mesh"):
    """메시를 다양한 형식으로 저장"""
    if mesh is None:
        return []
    ...
    # OBJ 형식
    mesh_path = os.path.join(output_dir, f"{base_name}.obj")
    o3d.io.write_triangle_mesh(mesh_path, mesh)
    saved_files.append(mesh_path)
    print(f"메시 저장 (OBJ): {mesh_path}")
    
    # PLY 형식
    mesh_ply_path = os.path.join(output_dir, f"{base_name}.ply")
    o3d.io.write_triangle_mesh(mesh_ply_path, mesh)
    saved_files.append(mesh_ply_path)
    print(f"메시 저장 (PLY): {mesh_ply_path}")
    
    # STL 형식
    mesh_stl_path = os.path.join(output_dir, f"{base_name}.stl")
    try:
        ...
        if success:
            saved_files.append(mesh_stl_path)
            print(f"메시 저장 (STL): {mesh_stl_path}")
    except Exception as e:
...
def visualize_results(merged_cloud, mesh):
    """결과 시각화"""
    print("\n=== 3D 시각화 ===")
    
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name="3D Body Model (Point-to-Plane ICP)", width=1024, height=768)
...
def main():
    """메인 실행 함수"""
    print("="*60)
    print("     Point-to-Plane ICP 기반 3D 신체 모델 생성 시스템")
    print("="*60)
    ...
    try:
        ...
        # 2단계: Point-to-Plane ICP 기반 포인트 클라우드 정렬
        aligned_clouds = align_point_clouds_all(point_clouds)
        ...
        # 4단계: 메시 생성
        print("\n=== 메시 생성 ===")
        print("포인트 클라우드를 고품질 메시로 변환 중...")
        mesh = create_mesh_from_pointcloud(merged_cloud)
        
        # 5단계: 메시 저장
        if mesh is not None:
            saved_files = save_mesh_files(mesh, output_dir, "body_mesh_p2p_icp")
            print(f"\n총 {len(saved_files)}개 파일이 저장되었습니다.")
        
        # 6단계: 결과 시각화
        visualize_results(merged_cloud, mesh)
        
        print("\n="*30)
        print("     3D 모델 생성이 완료되었습니다!")
        print("="*30)
